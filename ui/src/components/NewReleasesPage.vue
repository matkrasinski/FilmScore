<template>
  <div id="priceList" class="flex flex-col bg-slate-50 pt-2 shadow">
    ...
    <div class="text-center">
      <h2 v-if="this.movies.length != 0" class="text-gray-500 italic">Page {{ currentPage + 1}} out of {{ maxPages + 1}}</h2>
    </div>
    ...
    <div id="con" class="container pb-32">
      ...
      <div class="grid place-items-center w-full grid-cols-1 gap-64 md:grid-cols-2 lg:grid-cols-4 my-32 text-center">
        ...
        :circleColor="'#0b3f5d'"
        :scoreName="'predicted_rating'"
        @click="this.$router.push({ path: '/movies', query: { id: movie.tmdb_id, scoreType: 'predicted_rating' } });"
        ></movie-tile>
      </div>
...
export default {
  ...
  methods: {
    async loadMovies(page) {
      axios.post(`http://localhost:5000/db/movies/new/pages?page=${page}&size=${this.pageSize}&query=${this.query}`, [this.genres, this.sortBy], {
        headers: {
